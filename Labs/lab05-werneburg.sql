...
select orderdate, year(orderdate),month(orderdate),day(orderdate) from sales.orders;
...
select e.empid, max(e.orderdate) as maxorderdate from sales.orders e group by e.empid;
...
FROM sales.orders o1
   inner join 
   (select e.empid, max(e.orderdate) as maxorderdate from sales.orders e group by e.empid) as o2
...
select o1.orderid, o1.orderdate, o1.custid, o1.empid, ROW_NUMBER() OVER(Order by orderdate, orderid) as rownum from sales.orders o1;
...
select o2.orderid, o2.orderdate,o2.custid, o2.empid, o2.rownum from o2 where rownum between 11 and 20;
...
drop view if exists jay;
go

create view dbo.jay as
...
-- Write a query against Sales.VEmpOrders (renamed jay)
-- that returns the running qty for each employee and year
-- Tables involved: TSQLV4 database, Sales.VEmpOrders view


--try to write a scalar valued query against jay for sum(qty)

select sum(x.qty) from dbo.jay x;

select * from dbo.jay order by empid, orderyear;

select j.empid, j.orderyear, j.qty, 
  (select sum(j1.qty) as runqty from dbo.jay j1 where j.empid = j1.empid and j1.orderyear <=j.orderyear) as runqty
  from jay j order by j.empid, j.orderyear;  
